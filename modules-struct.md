...
Например, если мы используем только Браузер и действие у нас по сути одно, то логика размещается в файле _.js_ формата.
Вы можете открыть модуль AjaxReCaptcha, он построен ровно таким образом.
Основной файл - _browser.js_. Экшен только один и совершается без использования конструктора действий.

Этот файл подключается к модулю через манифест, в разделе **browser**. 
Но о разделах поговорим позже.

Обычно кроме минимального набора, существуют файлы действий. Структура такая:
